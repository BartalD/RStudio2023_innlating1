...
date: "Innlating í Moodle: seinast um kvøldið 7. februar 2023. Endurbrúka eisini fílnavnið á hesum template-skjalinum, sum tit lata inn, har tit seta tykkara forbókstavir inn fyri NN1_NN2_NN3_NN4."
...
akfar_tils <- akfar %>%
  group_by(ár, sýsla) %>%
  summarise(n_sýslur = sum(tilsamans)) %>%
  summarise(tils_sýslur = sum(n_sýslur))
akfar_tils
```
...
```{r}
# ár, slag, MWh
elorka <- read.csv2("data/elorka.csv", sep=",")
```
...
```{r}
elorka_perioda <- elorka %>% add_column("period" = cut_interval(x=elorka$ár, width=0, n=3, dig.lab=5))
elorka_perioda
```
...
event <- replicate(10000, {
  ...
  sum(bag == "Blue") == 1 & sum(bag == "Yellow") == 1 & sum(bag == "Red") == 1 & sum(bag == "Orange") == 1 & sum(bag == "Green") == 1 & sum(bag == "Brown") == 1
})
mean(event)
```
...
```{r}
dice_factor <- as.factor(table(sum_dice))
dice_factor
```
...
```{r}
ggplot(two_dice, aes(x = value, y = after_stat(prop))) +
  geom_bar(width = 0.75)
```
...
```{r}
p_sum <- prop.table(table(two_dice))
c_sum <- cumsum(p_sum)
c_sum
```
